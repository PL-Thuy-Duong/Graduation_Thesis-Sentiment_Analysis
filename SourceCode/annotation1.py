...
class emotic_train:
    ...
    def set_bbox(self, person_bbox):
        self.bbox = self.validate_bbox(
            np.array(person_bbox).flatten().tolist())
...
class emotic_test:
    ...
    def set_comb_cat(self, person_comb_cat):
        if self.cat_annotators != 0:
            self.comb_cat = [np.array(c).flatten().tolist()[0]
                             for c in person_comb_cat[0]]
        else:
            self.comb_cat = []

    def set_comb_cont(self, person_comb_cont):
        if self.cont_annotators != 0:
            comb_cont = [np.array(c).flatten().tolist()[0]
                         for c in person_comb_cont[0]]
            self.comb_cont = [np.array(c).flatten().tolist()[0]
                              for c in comb_cont[0]]
        else:
            self.comb_cont = []
...
def draw_bbox(image, bbox, color=(255, 0, 0), thickness=2):
    ...
    :param color: Màu của bbox (B, G, R)
    :param thickness: Độ dày của đường viền bbox
    :return: Ảnh đã được vẽ bbox
    """
    x1, y1, x2, y2 = bbox
...
def prepare_data(data_mat, data_path_src, save_dir, dataset_type='train', generate_npy=False, save_json=True, debug_mode=False):
    data_set = list()
    json_data = []
    processed_images = 0
    MAX_IMAGES = 25000
    ...
    for ex_idx, ex in enumerate(data_mat[0]):
        ...
        for person in range(nop):
            ...
            try:
                ...
                context_with_bbox = draw_bbox(context.copy(), et.bbox)
                
                bbox_image_folder = os.path.join(save_dir, dataset_type)
                os.makedirs(bbox_image_folder, exist_ok=True)
                
                # Đặt tên file ảnh với bbox có thêm chỉ số person
                bbox_image_filename = f"{os.path.splitext(et.filename)[0]}_{idx}.jpg"
                bbox_image_path = os.path.join(bbox_image_folder, bbox_image_filename)
                cv2.imwrite(bbox_image_path, context_with_bbox)
                
                print(f"Saved image with bbox: {bbox_image_path}")
                processed_images += 1



                # Xử lý context
                body = context[et.bbox[1]:et.bbox[3],
                                et.bbox[0]:et.bbox[2]].copy()
                ...
                if debug_mode:
                    print(
                        f'breaking at idx={ex_idx}, {idx} due to exception={e}')
                continue
            ...
            if save_json:
                json_entry = {
                    ...
                    'Image Size': et.im_size,
                    'BBox': et.bbox,
                    'Categorical_Labels': et.cat,
                    'Continuous_Labels': et.cont,
                    'Gender': et.gender,
                    'Age': et.age
                }
            ...
            if debug_mode and idx >= 104:
                print(
                    f' ######## Breaking data prep step {idx}, {ex_idx} ######')
                break
        if debug_mode and idx >= 104:
            break

    print(
        f"To break: {to_break}, Path not exist: {path_not_exist}, Cat/Cont zero: {cat_cont_zero}")

    csv_path = os.path.join(save_dir, f"{dataset_type}.csv")
    with open(csv_path, 'w') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', dialect='excel')
        row = ['Index', 'Folder', 'Filename', 'Image Size', 'BBox',
               'Categorical_Labels', 'Continuous_Labels', 'Gender', 'Age']
        filewriter.writerow(row)
        for idx, ex in enumerate(data_set):
            if dataset_type == 'train':
                row = [idx, ex.folder, ex.filename, ex.im_size,
                       ex.bbox, ex.cat, ex.cont, ex.gender, ex.age]
            else:
                row = [idx, ex.folder, ex.filename, ex.im_size, ex.bbox,
                       ex.comb_cat, ex.comb_cont, ex.gender, ex.age]
            filewriter.writerow(row)
    print(f'Wrote file {csv_path}')


    if save_json:
        json_path = os.path.join(save_dir, f"{dataset_type}.json")
    ...
    if generate_npy:
        ...
        print(
            f"Data set size: {len(data_set)}, Context shape: {context_arr.shape}, Body shape: {body_arr.shape}")
        np.save(os.path.join(
            save_dir, f'{dataset_type}_context_arr.npy'), context_arr)
        np.save(os.path.join(
            save_dir, f'{dataset_type}_body_arr.npy'), body_arr)
        np.save(os.path.join(save_dir, f'{dataset_type}_cat_arr.npy'), cat_arr)
        np.save(os.path.join(
            save_dir, f'{dataset_type}_cont_arr.npy'), cont_arr)
        print(
            f"Shapes - Context: {context_arr.shape}, Body: {body_arr.shape}, Cat: {cat_arr.shape}, Cont: {cont_arr.shape}")
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, required=True,
                        help='Path to Emotic data and annotations')
...
if __name__ == '__main__':
    args = parse_args()
    ann_path_src = os.path.join(
        args.data_dir, 'Annotations', 'Annotations.mat')
    ...
    for label in labels:
        ...
        prepare_data(data_mat, data_path_src, save_path, dataset_type=label,
                     generate_npy=args.generate_npy, debug_mode=args.debug_mode)
